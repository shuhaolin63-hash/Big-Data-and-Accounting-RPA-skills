...
def start():
    ...
    # 关闭字幕
    try:
        Debug_Block_Start('canvas-node-_xmiip2nv')
        ...
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_xmiip2nv', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_xmiip2nv')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'关闭字幕', 'id':'canvas-node-_xmiip2nv', 'time':block_start_time, 'error': error, 'description':r'''关闭已出现的字幕''', 'success':node_success_flag, 'variables':{}})
    # 调用自定义脚本
    try:
        Debug_Block_Start('canvas-node-_kxjv6eam')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_kxjv6eam', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_kxjv6eam')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'调用自定义脚本', 'id':'canvas-node-_kxjv6eam', 'time':block_start_time, 'error': error, 'description':r'''调用自定义脚本''', 'success':node_success_flag, 'variables':{}})
