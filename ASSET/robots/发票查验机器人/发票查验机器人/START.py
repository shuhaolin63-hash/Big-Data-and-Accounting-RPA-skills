...
def start():
    ...
    # 出现字幕
    try:
        Debug_Block_Start('canvas-node-_n66x5pb0')
        ...
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_n66x5pb0', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_n66x5pb0')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'出现字幕', 'id':'canvas-node-_n66x5pb0', 'time':block_start_time, 'error': error, 'description':r'''屏幕下方出现字幕 "机器人开始运行，请输入实践平台账号及密码！"''', 'success':node_success_flag, 'variables':{"size_changer": str(size_changer)}})
        if (error is not None):
            sys.exit(1)
    # 调用自定义脚本
    try:
        Debug_Block_Start('canvas-node-_kwvre3o7')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_kwvre3o7', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_kwvre3o7')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'调用自定义脚本', 'id':'canvas-node-_kwvre3o7', 'time':block_start_time, 'error': error, 'description':r'''调用自定义脚本''', 'success':node_success_flag, 'variables':{}})
        if (error is not None):
            sys.exit(1)
    # 设置变量值
    try:
        Debug_Block_Start('canvas-node-_d6156v79')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_d6156v79', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_d6156v79')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'设置变量值', 'id':'canvas-node-_d6156v79', 'time':block_start_time, 'error': error, 'description':r'''设置变量 username 的值为 username''', 'success':node_success_flag, 'variables':{"username": str(username)}})
        if (error is not None):
            sys.exit(1)
    # 设置变量值
    try:
        Debug_Block_Start('canvas-node-_kxivoqq6')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_kxivoqq6', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_kxivoqq6')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'设置变量值', 'id':'canvas-node-_kxivoqq6', 'time':block_start_time, 'error': error, 'description':r'''设置变量 password 的值为 password''', 'success':node_success_flag, 'variables':{"password": str(password)}})
        if (error is not None):
            sys.exit(1)
    # 出现字幕
    try:
        Debug_Block_Start('canvas-node-_kxivoqq8')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_kxivoqq8', error, False)
    finally: 
        if node_success_flag == True:
            sleep(3)
            Debug_Block_Success('canvas-node-_kxivoqq8')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'出现字幕', 'id':'canvas-node-_kxivoqq8', 'time':block_start_time, 'error': error, 'description':r'''屏幕下方出现字幕 "机器人正在自动识别测评任务！"''', 'success':node_success_flag, 'variables':{"size_changer": str(size_changer)}})
        if (error is not None):
            sys.exit(1)
    # 调用自定义脚本
    try:
        Debug_Block_Start('canvas-node-_kxivoqq9')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_kxivoqq9', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_kxivoqq9')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'调用自定义脚本', 'id':'canvas-node-_kxivoqq9', 'time':block_start_time, 'error': error, 'description':r'''调用自定义脚本''', 'success':node_success_flag, 'variables':{}})
        if (error is not None):
            sys.exit(1)
    # 设置变量值
    try:
        Debug_Block_Start('canvas-node-_l2ygnhpb')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_l2ygnhpb', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_l2ygnhpb')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'设置变量值', 'id':'canvas-node-_l2ygnhpb', 'time':block_start_time, 'error': error, 'description':r'''设置变量 fp_excel_path 的值为 path or fp_excel_path''', 'success':node_success_flag, 'variables':{"fp_excel_path": str(fp_excel_path)}})
        if (error is not None):
            sys.exit(1)
    # 关闭字幕
    try:
        Debug_Block_Start('canvas-node-_kxivoqqa')
        node_success_flag = True
        block_start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        node_success_flag = False
        error = '{0}'.format(e)
        Debug_Block_Error('canvas-node-_kxivoqqa', error, False)
    finally: 
        if node_success_flag == True:
            Debug_Block_Success('canvas-node-_kxivoqqa')
            error = None
        rpa_v33.agent.send('VisualDebug', 'DetailDebugLog', {'name':'关闭字幕', 'id':'canvas-node-_kxivoqqa', 'time':block_start_time, 'error': error, 'description':r'''''', 'success':node_success_flag, 'variables':{}})